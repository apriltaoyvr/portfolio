  ...
  return (
    ...
    >
        ...
        {isOpen && (
          <>
            ...
            >
              ...
              >
                {Object.entries(languages).map(([l, label]) => (
                  <option
                    key={l}
                    value={useTranslatedPath(l as Locale)(`/${route}`)}
                    className='bg-black'
                  >
                    {label}
                  </option>
            ...
            <motion.dialog
              ...
              className='lg:hidden fixed inset-0 z-50 m-0 max-w-none max-h-none w-screen h-screen flex flex-col gap-4 bg-zinc-900/20 backdrop-blur-md'
            >
              <div className='fixed bottom-20 sm:bottom-32 left-4 sm:left-8 md:px-4 flex flex-col items-start justify-center gap-4'>
                {route !== 'about' && (
      ...
      <button
        ...
        className='w-6 my-2 sm:w-8 h-fit text-white cursor-pointer hover:outline-2 focus-visible:outline-2 outline-offset-1 outline-zinc-100/20 focus-visible:outline-white rounded-lg'
      >
        <svg
          xmlns='http://www.w3.org/2000/svg'
          width='32'
